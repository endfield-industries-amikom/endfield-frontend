...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  ]).then(([schematicsRes, itemsRes, warehousesRes]) => {
    const allItems = itemsRes.data.data ?? [];
    return {
      schematics: schematicsRes.data.data,
      productOptions: allItems,
      outputProductOptions: allItems.filter((p: ItemOption & { isManufactureable?: boolean }) => p.isManufactureable !== false),
    ...
    <Box>
      ...
      <Suspense fallback={<SkeletonCards />}>
        <Await resolve={(loaderData as any).data}>
          {({ schematics, productOptions, outputProductOptions, warehouseOptions }: { schematics: ProductionSchematic[]; productOptions: ItemOption[]; outputProductOptions: ItemOption[]; warehouseOptions: WarehouseOption[] }) => {
            const productMap = new Map(productOptions.map((p: ItemOption) => [p.id, p]));
