...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const dataPromise = Promise.all([
  ...
  ]).then(([soRes, customersRes, regionsRes, productsRes]) => {
    ...
    return {
      salesOrders,
      customerOptions: customersRes.data.data,
...
export default function SalesOrdersSection({ loaderData, actionData }: Route.ComponentProps) {
  ...
  function updateLine(idx: number, field: keyof LineItem, value: string | number) {
    ...
    if (field === "itemId" && typeof value === "string") { const prod = productOptionsRef.current.find((p) => p.id === value); if (prod) updated[idx].unitPrice = prod.unitPrice; }
